...
export function claudeToDiscord(markdown: string): string {
 ...
 */
function detectCodeFenceLang(chunk: string): string {
...
export function formatToolCallDiscord(
  toolName: string,
  ...
  switch (toolName) {
    ...
    case "Write":
      return (
        `**Write File**: \`${inp.file_path || ""}\`\n` +
        `\`\`\`\n${(inp.content || "").slice(0, 500)}\n\`\`\``
      );

    case "NotebookEdit":
      return (
        `**Notebook Edit**: \`${inp.notebook_path || ""}\`\n` +
