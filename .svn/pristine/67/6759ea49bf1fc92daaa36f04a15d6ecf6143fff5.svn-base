...
import { snowflakeToNumeric, type DiscordBotConfig } from "./store.js";
...
async function downloadDiscordFile(url: string, maxBytes: number): Promise<Buffer> {
  ...
  if (contentLength > maxBytes) {
    throw new Error(
      `File too large (${(contentLength / 1024 / 1024).toFixed(1)} MB). Max is ${maxBytes / 1024 / 1024} MB.`
    );
  }
  ...
  if (buf.length > maxBytes) {
    throw new Error(
      `File too large (${(buf.length / 1024 / 1024).toFixed(1)} MB). Max is ${maxBytes / 1024 / 1024} MB.`
    );
  }
  return buf;
}
...
export function createDiscordWorker(
  ...
  const pendingFreeText = new Map<
    string,
    { resolve: (answer: string) => void; timer: NodeJS.Timeout; question: string; msgId: string }
  >();
  const pendingNgrokSetup = new Map<
    string,
    { port: number; timer: NodeJS.Timeout }
  >();
  ...
  // ---- Authorization check ----
  function isAuthorized(channelId: string, userId: string): boolean {
    if (isDiscordOwner(userId)) return true;
  ...
  // ---- Help text ----
  function getHelpText(chatId?: number): string {
  ...
  // ---- Tunnel helper ----
  async function openTunnelAndNotify(channel: TextChannel, chatId: number, port: number): Promise<void> {
    try {
      ...
      await channel.send(`Tunnel error: ${(err as Error).message}`).catch(() => {});
    }
  }
  ...
  function extractReplyContext(referencedMessage: Message | null): string {
    if (!referencedMessage) return "";
    const quoted = referencedMessage.content;
    if (!quoted) return "";
    const preview =
      quoted.length > REPLY_PREVIEW_MAX
  ...
  async function sendSessionHistory(channel: TextChannel, sessionId: string): Promise<void> {
    try {
  ...
  // ---- Core handlePrompt ----
  function handlePrompt(channelId: string, prompt: string, channel: TextChannel) {
    (async () => {
      ...
      const onAskUser = async (
        ...
        for (let i = 0; i < questions.length; i++) {
          ...
          const optionButtons: ButtonBuilder[] = [];
      ...
      const onToolApproval = async (
        ...
        const result = await new Promise<"allow" | "always" | "deny">(
          (resolve) => {
            const requestId = String(++approvalCounter);
      ...
      const onResult = async (result: {
        ...
        await channel
          .send(
            `${tokens.toLocaleString()} tokens | ${result.turns} turns | ${seconds}s | 📂 ${getRepoName(numericId)}`
          )
          .catch(() => {});
      };
  ...
  client.once("clientReady", async () => {
    ...
    const commands = [
      ...
      new SlashCommandBuilder()
        ...
        .addStringOption((opt) =>
          opt.setName("port").setDescription("Port number (e.g. 3000)").setRequired(false)
        ),
      ...
      new SlashCommandBuilder()
        ...
        .addSubcommand((sub) =>
          sub
            ...
            .addStringOption((opt) =>
              opt.setName("path").setDescription("Absolute path to the repo").setRequired(true)
            )
            .addStringOption((opt) =>
              opt.setName("alias").setDescription("Short name (optional)").setRequired(false)
            )
        )
        .addSubcommand((sub) =>
          sub.setName("list").setDescription("List all repos")
        )
        .addSubcommand((sub) =>
          sub
            .setName("switch")
            ...
            .addStringOption((opt) =>
              opt.setName("alias").setDescription("Repo alias").setRequired(true)
            )
        )
        .addSubcommand((sub) =>
          sub
  ...
  client.on("interactionCreate", async (interaction: Interaction) => {
      ...
      switch (interaction.commandName) {
        ...
        case "resume": {
          ...
          for (const s of sessions) {
            const dateStr =
              s.modifiedAt.toLocaleDateString("en-US", {
                month: "short",
                day: "numeric",
              }) +
              ", " +
        ...
        case "cron": {
          ...
          if (sub === "add") {
            ...
            if (!parsed) {
              await interaction
                .editReply(
                  "Could not parse schedule. Try being more specific, e.g. `/cron add task:daily 9am run tests`"
                )
                .catch(() => {});
              break;
            }
            ...
            await interaction
              .reply(
                `**Scheduled tasks for ${getRepoName(numericId)}**\n\n` +
                  lines.join("\n\n") +
        ...
        case "members": {
          ...
          const ownerList = config.DISCORD_OWNER_IDS.map(
            (id) => `  - ${id} (owner)`
          ).join("\n");
          if (allowed.length === 0) {
            await interaction
        ...
        case "repo": {
          ...
          if (sub === "add") {
            ...
            try {
              ...
              await interaction
                .reply({ content: `Failed: ${(err as Error).message}`, ephemeral: true })
                .catch(() => {});
            }
          } else if (sub === "list") {
            const repos = router.listRepos();
    ...
    if (interaction.isStringSelectMenu()) {
      ...
      if (interaction.customId === "mode_select") {
        const modeId = interaction.values[0] as import("./claude.js").PermissionMode;
        const label =
      ...
      if (interaction.customId === "repo_switch_select") {
        ...
        try {
          ...
          await interaction
            .update({ content: `Failed: ${(err as Error).message}`, components: [] })
            .catch(() => {});
        }
        return;
      }
  ...
  client.on("messageCreate", async (message: Message) => {
    ...
    // Authorization check
    if (!isAuthorized(channelId, userId)) return;
    ...
    if (message.attachments.size === 0 && message.embeds.length > 0) {
      for (const embed of message.embeds) {
        ...
        if (embed.thumbnail?.url && !embed.image) embedImageUrls.push(embed.thumbnail.url);
      }
    }
    ...
    if (message.attachments.size > 0 || embedImageUrls.length > 0) {
      ...
      for (const [, attachment] of message.attachments) {
        if (attachment.size > MAX_DOWNLOAD_BYTES) {
          await channel
            .send(
              `File too large: ${attachment.name} (${(attachment.size / 1024 / 1024).toFixed(1)} MB). Max is ${MAX_DOWNLOAD_BYTES / 1024 / 1024} MB.`
            )
            .catch(() => {});
          continue;
        }
        ...
        try {
          ...
          await channel.send(`Failed to download ${rawName}: ${(err as Error).message}`).catch(() => {});
        }
      }
      ...
      for (let i = 0; i < embedImageUrls.length; i++) {
        ...
        try {
          ...
          await channel.send(`Failed to download embedded image: ${(err as Error).message}`).catch(() => {});
        }
      }

      if (filePaths.length === 0) return;

      const isImage = filePaths.some((fp) =>
        /\.(png|jpe?g|gif|webp|bmp|svg)$/i.test(fp)
      );
    ...
    if (textCmd === "/repo" || textCmd.startsWith("/repo ")) {
      ...
      if (subCmd === "add") {
        ...
        try {
          ...
          await channel.send(`Failed: ${(err as Error).message}`).catch(() => {});
        }
        return;
      }
