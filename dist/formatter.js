...
export function claudeToTelegram(markdown) {
    let result = "";
    ...
    for (const part of parts) {
        if (part.startsWith("```")) {
            ...
            // Restore inline code
            text = text.replace(/\x00IC(\d+)\x00/g, (_, i) => `<code>${inlineCodes[Number(i)]}</code>`);
...
function getOpenTags(html) {
    ...
    while ((m = tagRe.exec(html)) !== null) {
        ...
        if (!PAIRED_TAGS.includes(tagName))
            continue;
        if (full.startsWith("</")) {
...
export function formatToolCall(toolName, input) {
    ...
    switch (toolName) {
        case "Bash":
            return (`<b>Bash Command</b>\n` +
                `<pre>${escapeHtml(inp.command || "")}</pre>`);
        case "Edit":
            return (`<b>Edit File</b>: <code>${escapeHtml(inp.file_path || "")}</code>\n` +
                `Old:\n<pre>${escapeHtml((inp.old_string || "").slice(0, 300))}</pre>\n` +
                `New:\n<pre>${escapeHtml((inp.new_string || "").slice(0, 300))}</pre>`);
        case "Write":
            return (`<b>Write File</b>: <code>${escapeHtml(inp.file_path || "")}</code>\n` +
                `<pre>${escapeHtml((inp.content || "").slice(0, 500))}</pre>`);
        case "NotebookEdit":
            return (`<b>Notebook Edit</b>: <code>${escapeHtml(inp.notebook_path || "")}</code>\n` +
