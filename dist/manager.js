...
export function createManager(callbacks) {
    ...
    function setConversation(chatId, state) {
    ...
    const helpText = "<b>Claude Multi-Bot Manager</b>\n\n" +
        "<b>Commands:</b>\n" +
    ...
    bot.command("add", async (ctx) => {
        ...
        if (!args) {
            ...
            await ctx.reply("Send me the bot token from @BotFather.\n\n" +
        ...
        if (spaceIdx === -1) {
            await ctx.reply("Usage: <code>/add TOKEN /path/to/repo</code>\n\n" +
                "Or just send <code>/add</code> for interactive setup.", { parse_mode: "HTML" });
            return;
        }
    ...
    bot.command("remove", async (ctx) => {
        ...
        try {
            ...
            console.error(`[manager] Remove @${arg} failed:`, err.message);
            await ctx.reply(`Error removing @${arg}. Check logs for details.`);
        }
    });
    async function addWorkerBot(ctx, token, dir) {
        ...
        try {
            ...
            console.error("[manager] Token validation failed:", err.message);
            await ctx.reply("Invalid bot token. Check it and try again.");
            return;
        }
        const workers = callbacks.getActiveWorkers();
        if (workers.has(botInfo.id)) {
            await ctx.reply(`Bot @${botInfo.username} is already active. Remove it first with /remove @${botInfo.username}`);
            return;
        }
        const botConfig = {
            id: botInfo.id,
        ...
        try {
            ...
            await ctx.reply(`Added @${escapeHtml(botInfo.username)}\n` +
                `Repo: <code>${escapeHtml(dir)}</code>\n\n` +
                `DM @${escapeHtml(botInfo.username)} to start working!`, { parse_mode: "HTML" });
        }
        catch (err) {
            console.error("[manager] Worker start failed:", err.message);
    ...
    bot.on("message:text", async (ctx) => {
        ...
        if (conv.step === "token") {
            if (!/^\d+:[A-Za-z0-9_-]+$/.test(text)) {
                await ctx.reply("That doesn't look like a valid bot token.\n" +
                    "It should look like: <code>123456:ABC-DEF...</code>\n\n" +
                    "Try again or send /cancel to abort.", { parse_mode: "HTML" });
                return;
            }
            setConversation(ctx.chat.id, { step: "path", token: text });
            await ctx.reply("Now send the full path to the repository.\n\nExample: <code>/Users/me/projects/my-app</code>", { parse_mode: "HTML" });
            return;
        }
        if (conv.step === "path") {
            clearConversation(ctx.chat.id);
