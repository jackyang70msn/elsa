...
import { claudeToTelegram, splitMessage, formatToolCall, escapeHtml, } from "./formatter.js";
...
export function createWorker(botConfig, router, tunnelManager, scheduleManager) {
    ...
    const helpText = `<b>${escapeHtml(repoName)}</b>\n` +
    ...
    bot.command("cost", async (ctx) => {
        ...
        await ctx.reply(`<b>Session tokens</b>\n` +
            ...
            `Total: ${total.toLocaleString()}`, { parse_mode: "HTML" });
    });
    ...
    bot.command("session", async (ctx) => {
        ...
        await ctx.reply(`<b>Session ID</b>\n<code>${sessionId}</code>\n\n` +
            `<b>Continue in CLI</b>\n` +
            `Run this from <code>${botConfig.workingDir}</code>:\n\n` +
            `<code>${cmd}</code>\n\n` +
            `Tap the command above to copy it.`, { parse_mode: "HTML" });
    });
    ...
    bot.command("repo", async (ctx) => {
        ...
        if (subCmd === "add") {
            if (!subArg) {
                await ctx.reply("<b>Add a Repository</b>\n\n" +
                    "<code>/repo add /path/to/repo [alias]</code>\n\n" +
                    "<b>Examples:</b>\n" +
                    "<code>/repo add /home/user/myproject</code>\n" +
                    "<code>/repo add /home/user/another my-app</code>", { parse_mode: "HTML" });
                return;
            }
            ...
            try {
                ...
                await ctx.reply(`<b>Repository Added</b>\n\n` +
                    `<b>Alias:</b> <code>${escapeHtml(newRepo.alias)}</code>\n` +
                    `<b>Path:</b> <code>${escapeHtml(newRepo.path)}</code>\n\n` +
                    `Use <code>/repo switch ${escapeHtml(newRepo.alias)}</code> to switch to this repo.`, { parse_mode: "HTML" });
            }
            catch (err) {
            ...
            const repos = router.listRepos();
            if (repos.length === 0) {
                await ctx.reply("<b>No repositories registered</b>\n\n" +
                    "Use <code>/repo add /path/to/repo [alias]</code> to add one.", { parse_mode: "HTML" });
                return;
            }
            ...
            const lines = repos
                ...
                .join("\n\n");
            await ctx.reply(`<b>Registered Repositories</b>\n\n${lines}\n\n` +
                `Use <code>/repo switch &lt;alias&gt;</code> to switch.`, { parse_mode: "HTML" });
        }
        else if (subCmd === "switch") {
            if (!subArg) {
                await ctx.reply("<b>Switch Repository</b>\n\n" +
                    "<code>/repo switch &lt;alias&gt;</code>\n\n" +
                    "Use <code>/repo list</code> to see available repositories.", { parse_mode: "HTML" });
                return;
            }
            ...
            await ctx.reply(`<b>Switched to Repository</b>\n\n` +
                `<b>Alias:</b> <code>${escapeHtml(repo.alias)}</code>\n` +
                `<b>Path:</b> <code>${escapeHtml(repo.path)}</code>`, { parse_mode: "HTML" });
        }
        else if (subCmd === "remove") {
            if (!subArg) {
                await ctx.reply("<b>Remove Repository</b>\n\n" +
            ...
            await ctx.reply(`<b>Repository Removed</b>\n\n` +
                `<code>${escapeHtml(repo.alias)}</code> has been removed.\n\n` +
                `<i>Note: The directory was not deleted, only unregistered from Elsa.</i>`, { parse_mode: "HTML" });
        }
        else {
            await ctx.reply("<b>Repository Management</b>\n\n" +
                ...
                "<code>/repo remove &lt;alias&gt;</code> — Unregister a repository", { parse_mode: "HTML" });
        }
    });
    ...
    bot.command("cron", async (ctx) => {
        ...
        if (subCmd === "add") {
            if (!subArg) {
                await ctx.reply("<b>新增排程任務</b>\n\n" +
                    "使用自然語言描述，例如：\n" +
                    "<code>/cron add 每天早上 9 點執行測試並修復錯誤</code>\n" +
                    "<code>/cron add every monday write changelog from last week's commits</code>\n" +
                    "<code>/cron add every 6 hours check for new dependency vulnerabilities</code>", { parse_mode: "HTML" });
                return;
            }
            ...
            await ctx.reply("<b>確認排程</b>\n\n" +
                `<b>時間：</b> ${escapeHtml(parsed.humanLabel)}${parsed.once ? " (一次性)" : ""}\n` +
                `<b>任務：</b> ${escapeHtml(parsed.prompt)}\n\n` +
                "<i>排程任務將自動執行，不需人工核准。</i>", { parse_mode: "HTML", reply_markup: keyboard });
        }
        else if (subCmd === "list") {
            ...
            await ctx.reply(`<b>${escapeHtml(repoName)} 的排程任務</b>\n\n` +
                lines.join("\n\n") +
                "\n\n使用 /cron del &lt;編號&gt; 移除。", { parse_mode: "HTML" });
        }
        else if (subCmd === "del") {
            if (!subArg) {
            ...
            await ctx.reply("<b>排程管理</b>\n\n" +
                "<code>/cron add [任務描述]</code> — 新增排程\n" +
                "<code>/cron list</code> — 列出排程\n" +
                "<code>/cron del [編號]</code> — 移除排程", { parse_mode: "HTML" });
        }
    });
    // --- Tunnel commands ---
    ...
    async function openTunnelAndNotify(chatId, port) {
        try {
            ...
            await bot.api.sendMessage(chatId, `Live preview: ${url}\n\nPort ${port}. Open on your phone!`, { reply_markup: keyboard });
        }
        catch (err) {
            await bot.api.sendMessage(chatId, `Tunnel error: ${err.message}`);
        }
    }
    ...
    bot.command("preview", async (ctx) => {
        ...
        if (arg) {
            ...
            if (!config.NGROK_AUTH_TOKEN) {
                ...
                await ctx.reply("To use live preview, you need an ngrok auth token.\n\n" +
    ...
    function handlePrompt(chatId, prompt, replyFn) {
        (async () => {
            if (router.isProcessing(chatId)) {
            ...
            const doEdit = async () => {
                ...
                if (!plainContent.trim() || plainContent === lastEditedText)
                    return;
            ...
            const onResult = async (result) => {
                ...
                await bot.api
                    .sendMessage(chatId, `${tokens.toLocaleString()} tokens | ${result.turns} turns | ${seconds}s | 📂 ${getRepoName(chatId)}`)
                    .catch(() => { });
            };
            const onError = async (error) => {
                ...
                if (errorMsg.includes("No conversation found") || errorMsg.includes("session") && errorMsg.includes("not found")) {
                    errorMsg = ("❌ Session not found. The conversation file may have been deleted or moved.\n\n" +
                        "Solutions:\n" +
                ...
                if (thinkingMsgId) {
                    try {
                        await bot.api.editMessageText(chatId, thinkingMsgId, errorMsg, { reply_markup: keyboard });
                    }
                    catch {
    ...
    function extractReplyContext(ctx) {
    ...
    async function sendSessionHistory(chatId, sessionId) {
        try {
    ...
    bot.on("message:text", (ctx) => {
        ...
        if (freeText) {
            ...
            bot.api.editMessageText(chatId, freeText.msgId, `<b>${escapeHtml(freeText.question)}</b>\n\nAnswer: <b>${escapeHtml(ctx.message.text)}</b>`, { parse_mode: "HTML" }).catch(() => { });
    ...
    bot.on("callback_query:data", async (ctx) => {
        ...
        if (data.startsWith("cron:confirm:") || data.startsWith("cron:cancel:")) {
            ...
            await ctx.editMessageText(`<b>排程已儲存</b>\n\n` +
                `<b>時間：</b> ${escapeHtml(schedule.humanLabel)}\n` +
                `<b>任務：</b> ${escapeHtml(schedule.prompt)}\n\n` +
                `使用 /cron list 查看，/cron del 移除。`, { parse_mode: "HTML" }).catch(() => { });
        ...
        if (modelMatch) {
            ...
            await ctx.editMessageText(`Model switched to <b>${label}</b>\nSession reset — next message uses the new model.`, { parse_mode: "HTML" }).catch(() => { });
        ...
        if (modeMatch) {
            ...
            await ctx.editMessageText(`Permission mode switched to <b>${label}</b>`, { parse_mode: "HTML" }).catch(() => { });
            await ctx.answerCallbackQuery(`Switched to ${label}`).catch(() => { });
            return;
        }
        ...
        if (resumeMatch) {
            ...
            await ctx.editMessageText(`Session resumed: <code>${sessionId}</code>\n\nSend a message to continue.`, { parse_mode: "HTML" }).catch(() => { });
        ...
        if (data.startsWith("answer:")) {
            ...
            if (optPart === "other") {
                ...
                await ctx.editMessageText(`<b>${escapeHtml(pending.question)}</b>\n\nType your answer:`, { parse_mode: "HTML" }).catch(() => { });
                const chatId = ctx.chat.id;
        ...
        if (data.startsWith("retry:")) {
            ...
            handlePrompt(chatId, lastPrompt, (text) => bot.api.sendMessage(chatId, text));
            return;
        }
        ...
        clearTimeout(pending.timer);
        pendingApprovals.delete(requestId);
        const result = action === "approve" ? "allow" :
            action === "alwaysallow" ? "always" :
                "deny";
        pending.resolve(result);
        const statusLabel = result === "allow" ? "APPROVED" :
        ...
        const answerText = result === "allow" ? "Approved" :
