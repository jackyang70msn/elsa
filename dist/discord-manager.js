import { Client, GatewayIntentBits, Partials, SlashCommandBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, ActionRowBuilder, } from "discord.js";
...
export function createDiscordManager(token, guildId, callbacks) {
    ...
    client.once("clientReady", async () => {
        try {
            ...
            console.error("[discord-manager] Failed to register slash commands:", err.message);
        }
    });
    // --- Interaction handler ---
    client.on("interactionCreate", async (interaction) => {
        try {
            ...
            console.error("[discord-manager] Interaction error:", err.message);
        }
    });
    // --- Command handlers ---
    ...
    async function handleAdd(interaction) {
        ...
        const pathRow = new ActionRowBuilder().addComponents(pathInput);
    ...
    async function handleAddModal(interaction) {
        ...
        try {
            ...
            console.error("[discord-manager] Token validation failed:", err.message);
        ...
        try {
            ...
            await interaction.editReply({
                content: `Added **@${escapeMarkdown(botInfo.username)}**\n` +
                    `Repo: \`${repoPath}\`\n\n` +
                    `Message @${escapeMarkdown(botInfo.username)} to start working!`,
            }).catch(() => { });
        }
        catch (err) {
            console.error("[discord-manager] Worker start failed:", err.message);
            await interaction.editReply({
    ...
    async function handleRemove(interaction) {
        ...
        try {
            ...
            console.error(`[discord-manager] Remove @${arg} failed:`, err.message);
            await interaction.reply({
    ...
    async function handleHelp(interaction) {
        const helpText = "**Claude Multi-Bot Manager (Discord)**\n\n" +
            "**Commands:**\n" +
