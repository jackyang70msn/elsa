...
const activeDiscordWorkers = new Map();
let scheduleManager;
const lastWorkerError = new Map(); // botId → timestamp of last polling error
const lastDiscordWorkerError = new Map();
const RESTART_COOLDOWN_MS = 120_000; // wait 2 minutes before restarting a failed worker
let healthCheckTimer = null;
let discordManagerClient = null;
...
async function startWorker(botConfig) {
...
async function stopWorker(botId) {
...
async function startDiscordWorker(botConfig) {
...
async function stopDiscordWorker(botId) {
...
function getActiveDiscordWorkers() {
    const result = new Map();
...
async function main() {
    ...
    scheduleManager = new ScheduleManager(async (schedule) => {
        ...
        if (platform === "discord" && channelId) {
            ...
            const channel = await discordWorker.client.channels.fetch(channelId).catch((err) => {
    ...
    if (config.TELEGRAM_BOT_TOKEN && config.TELEGRAM_OWNER_IDS.length > 0) {
        ...
        await Promise.allSettled(savedBots.map(botConfig => startWorker(botConfig).catch(err => console.error(`Failed to restore worker @${botConfig.username}:`, err))));
    }
    else {
    ...
    if (config.DISCORD_BOT_TOKEN && config.DISCORD_OWNER_IDS.length > 0) {
        try {
            ...
            discordManagerClient = createDiscordManager(config.DISCORD_BOT_TOKEN, guildId, { startWorker: startDiscordWorker, stopWorker: stopDiscordWorker, getActiveWorkers: getActiveDiscordWorkers });
            ...
            if (savedDiscordBots.length > 0) {
                await Promise.allSettled(savedDiscordBots.map(bot => startDiscordWorker(bot).catch(err => console.error(`Failed to restore Discord worker ${bot.username}:`, err))));
            }
            console.log(`Active Discord workers: ${activeDiscordWorkers.size}`);
        }
        catch (err) {
            console.error("[discord] Failed to initialize Discord:", err.message);
        }
    }
    ...
    healthCheckTimer = setInterval(async () => {
        ...
        if (deadConfigs.length > 0) {
            await Promise.allSettled(deadConfigs.map(async (cfg) => {
                try {
                    ...
                    console.error(`[${cfg.username}] Restart failed: ${err.message}`);
                }
            }));
        }
        ...
        if (missing.length > 0) {
            await Promise.allSettled(missing.map(async (botConfig) => {
                try {
                    ...
                    console.error(`[${botConfig.username}] Recovery failed: ${err.message}`);
                    lastWorkerError.set(botConfig.id, Date.now());
                }
            }));
        }
        ...
        if (activeDiscordWorkers.size > 0) {
            ...
            if (deadDiscord.length > 0) {
                await Promise.allSettled(deadDiscord.map(async (cfg) => {
                    try {
                        ...
                        console.error(`[${cfg.username}] Discord restart failed: ${err.message}`);
                    }
                }));
            }
            ...
            const missingDiscord = savedDiscordBots.filter(b => {
                if (activeDiscordWorkers.has(b.id))
                    return false;
            ...
            if (missingDiscord.length > 0) {
                await Promise.allSettled(missingDiscord.map(async (bot) => {
                    try {
                        ...
                        console.error(`[${bot.username}] Discord recovery failed: ${err.message}`);
